import * as schemaValidationday4 from './schema-day4.json';
...
const passportFormated = passports.map(p=>({
  ...p,
  eyr: Number(p.eyr),
  iyr: Number(p.iyr),
  // pid: Number(p.pid),
...
const requiredProps= ['ecl', 'pid', 'eyr', 'hcl', 'byr', 'iyr', 'hgt']
const validPassportsNb = passportFormated.reduce((acc, passport)=>{
  const pKeys = Object.keys(passport);
  const v = new Validator();
  const isValid = requiredProps.every(req=> pKeys.includes(req));
  const res = v.validate(passport, schemaValidationday4); //?
  if(isValid && !res.errors.length) return acc+1;
  return acc;
}, 0)
validPassportsNb //? 
